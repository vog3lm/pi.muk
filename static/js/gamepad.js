    ...
    function Gamepad() {
        ...
        this._keyMapping = {
            gamepad: {
                ...
                'button_4': 4, // Y
                // 5
                'shoulder_top_left': 6, // L1
                'shoulder_top_right': 7, // R1
                'shoulder_bottom_left': 8, // L2
                'shoulder_bottom_right': 9, // R2
                'select': 10,
                'start': 11, 
                // 12
                'stick_button_left': 13, // TL
                'stick_button_right': 14, // TR
                'vendor': 15 // ?
            },
            axes: {
                'stick_axis_left': [0, 2],
                'stick_axis_right': [2, 4],
                'shoulder_bottom_axis': [4, 6],
                'd_pad_axis': [6, 8],
            },
            keyboard: {
                'button_1': 32,
    ...
    Gamepad.prototype._handleGamepadAxisEventListener = function (controller) {
        ...
        if (controller && controller.connected) {
            Object.keys(self._keyMapping.axes).forEach(function (key) {
                ...
                if('shoulder_bottom_axis' === key && (axes[0] > self._threshold || axes[1] > self._threshold)){
                    ...
                    /* event.value stucks on last seen */
                } else if ('shoulder_bottom_axis' === key){
                    // avoid hold == -1, do nothing                    
                } else if (Math.abs(axes[0]) > self._threshold || Math.abs(axes[1]) > self._threshold) {
                    self._events.axes[controller.index][key] = {
                        pressed: self._events.axes[controller.index][key] ? false : true,
                        hold: self._events.axes[controller.index][key] ? true : false,
                        released: false,
